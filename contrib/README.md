...
| **schema** | `contrib/schema/` | a record seen by field name — `schema_fields` / `schema_text` / `schema_with`, over the name-keyed view the compiler already synthesises for `to_json`. Lets a form be generated from a record declaration instead of written out beside it. Needs `of_json_like` (v0.1.183) to be generic over the record type. | top-level |
| **state** | `contrib/state/` | mutable state that says what it is: `cell_new` / `cell_get` / `cell_set` for a slot nothing is derived from, and `store_new` / `store_watch` / `store_update` for state the screen is a function of. Replaces the one-slot-vec trick the browser clients had ten of. | top-level |
| **inc** | `contrib/inc/` | recompute only what changed: `inc_declare` (a node and what it reads) / `inc_touch` (an input changed) / `inc_order` (the dirty nodes, dependencies first) / `inc_put` / `inc_peek`, plus `inc_recomputes` so the cost is a number the engine reports rather than a claim. Dependencies are **declared, not traced** — a tracing engine has to call back into the consumer's compute function, which is a different design. Two consumers and the negative controls are in `test/inc/`; the gate is `scripts/inc_check.sh`. | top-level |
| **font** | `contrib/font/` | a TrueType file as widths and outlines — `Font.load` / `text_width` / `glyph_quads` / `quads_cover` / `text_raster`. Imports nothing. glyf outlines and cmap format 4 only; no `.ttc`, no `gvar`. From the mbrowse dogfood, where its gates stay because the oracle is a browser. | ✓ module (interp / C / RV32I; **wasm and LLVM reject it**, both inside the contour walker) |
