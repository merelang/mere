...
| **50f-2** | Browser integration — `contrib/site/playground/selfhost-fmt.mere` wires `dom + parser + fmt` into one Wasm module. The textarea on `selfhost-fmt.html` feeds `tokenize → parse_expr → format_expr`, with the result written back through `dom_set_text`. 7-case node smoke test (arithmetic precedence / let-in / curried lambda / factorial / match / list / range) all round-trip cleanly. | **complete** |
| **50g** | `format_program` in `contrib/fmt/fmt.mere` — `fmt_top_let` / `fmt_top_let_rec` / `fmt_top_type` ported from `lib/formatter.ml` so the browser bridge can accept full Mere files (decls + main). 6 new program-level demos + 5 new node smoke-test cases (`let X = …; … ; main`, variant decls, decls-only) all round-trip; a non-trivial multi-decl program (`type opt = … ; let x = … ; let rec sum = … ; sum [1, 2, 3]`) cross-validates **byte-identical to OCaml-side `mere fmt`**. Records / extern / view / signature still deferred. | **complete** |
| **50h** | Self-host fmt cross-validation in `dune runtest` — `test/test_basic.ml` adds 13 cases that feed an input through both `Pipeline.parse_program + Formatter.format_program` (OCaml) and the self-host pipeline (Mere `tokenize + parse_decls + format_program` via `Pipeline.process` over a synthesized bridge script that reads the input via `read_file`). Each case asserts byte-identical output. Covers expressions, top-level decls, mutual `let rec ... and`, variant types, annotated lambdas, list / range / match. CI now catches any self-host regression vs the OCaml reference. | **complete** |
| **50i** | Records — adds `TopRecord` variant to `ast.mere`, `fmt_top_record` to `fmt.mere`, and 5 new pieces to `parser.mere`: record decl (`type Point = { x: int, y: int };`), record literal (`Point { x = 1, y = 2 }`), record update (`{ base | x = e, ... }`), field access (`e.f`, postfix-chained left-associatively above application), and record pattern (`Point { x = 0, y = 0 }`). 12 new parser demos + 5 cross-validation cases all byte-identical across interp / Wasm / C and against OCaml-side `mere fmt`. Browser bridge picks up records for free. | **complete** |
| **50j** | "Self-host parses self-host" — fills the last few gaps so `contrib/parser/{lexer,parser}.mere` and `contrib/fmt/fmt.mere` can be parsed by the self-host parser: type alias decls (`type pos_token = int * token;` ⇒ `TopTypeAlias`), `import "..."` declarations (recognised and skipped — the OCaml side resolves them, the self-host trusts the caller to have done so), and atom-style constructor payload in patterns (`Some t`, not just `Some (t)`; safe because pattern context has no function-application ambiguity). Also fixes a pre-existing bug in `escape_string_for_fmt` where the opening quote of a string literal was emitting `{` instead of `"`. `dune runtest` adds 3 new "self-host parses X" cases (1602 total). | **complete** (this commit) |
