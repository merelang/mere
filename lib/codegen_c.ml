...
let flatten_module_dots = Monomorph.flatten_module_dots
...
(* v0.1.291: does this program use a region block at all? A closure only needs a
   ...
   Programs that do not use regions therefore keep the pre-v0.1.290 closure: two
   pointers, an env in the default region, a shallow copy. That matters because
   the copier is a third pointer in a value that is passed BY VALUE through every
   frame of an interpreter's dispatch, and an interpreter with a 512 MB stack
   measured one of CRuby's bootstraptest pairs going from pass to
   "stack overflow (recursion too deep)" -- the recursion was already at the edge
   and the extra pointer per frame crossed it. Charging that to programs which
   cannot benefit is the wrong trade. *)
let program_uses_regions () = !Typer.saw_region_block
...
let ty_is_concrete = Monomorph.ty_is_concrete
...
(* Phase 23.3: mangle a fn name with its concrete arrow type tag, e.g.
   ...
   specialization. Defined in Monomorph so that every backend's call sites
   name the same instance. *)
let mangled_inst_name = Monomorph.mangled_inst_name
...
let pattern_vars = Monomorph.pattern_vars
...
let rec emit_expr (e : Ast.expr) : string =
  ...
  | Ast.Let (pat0, value0, _) when
      (* Q-128: A CHAIN OF `let`s BECOMES ONE STATEMENT EXPRESSION, NOT A TOWER OF THEM.
         ...
         Clang's default limit is 256 and Ubuntu's clang 18 enforces it where Apple's
         does not, so programs built on a Mac and failed on Linux: m3d's `main` sat at
         533 and its CI was red at the first gate for the life of the project, and
      ...
      (match pat0.pnode, value0.Ast.node with
       | Ast.P_var n, Ast.Fun _ -> Hashtbl.mem inner_lifts n
       | Ast.P_var n, _ ->
         (not (Hashtbl.mem top_globals n))
         && (match value0.Ast.node with
             | Ast.App ({ Ast.node = Ast.Var "owned_vec_new"; _ }, _) -> false
             | _ -> true)
       | (Ast.P_wild | Ast.P_unit), _ -> true
       | _ -> false) ->
    let stmts = ref [] in
    ...
    let rec chain (cur : Ast.expr) : string =
      match cur.Ast.node with
      | Ast.Let (pat, value, body) ->
        (match pat.Ast.pnode with
         | Ast.P_var name when
             (match value.Ast.node with Ast.Fun _ -> true | _ -> false)
             && Hashtbl.mem inner_lifts name ->
           c_tail_pos := __in_tail; chain body
         | Ast.P_var name
           when (not (Hashtbl.mem bound name))
           ...
           (* The two-step form the nested shape uses, for the same reason: the
              initializer of `let xs = f xs` must still read the OUTER xs. *)
           stmts := Printf.sprintf
             "__auto_type __let_tmp_%s = %s; __auto_type %s = __let_tmp_%s;"
             tmp value_c safe tmp :: !stmts;
           c_tail_pos := __in_tail; chain body
         | Ast.P_wild | Ast.P_unit ->
           c_tail_pos := false;
           let value_c = emit_expr value in
           stmts := Printf.sprintf "(void)(%s);" value_c :: !stmts;
           c_tail_pos := __in_tail; chain body
         | _ -> c_tail_pos := __in_tail; emit_expr cur)
      | _ -> c_tail_pos := __in_tail; emit_expr cur
    in
    let final =
      try chain e
      with ex ->
        current_var_types := prev_types; current_env_subst := prev_subst; raise ex
    in
    current_var_types := prev_types;
    current_env_subst := prev_subst;
    if !stmts = [] then final
    else Printf.sprintf "({ %s %s; })"
           (String.concat " " (List.rev !stmts)) final
  | Ast.Let (pat, value, body) ->
  ...
  | Ast.App (f, arg) ->
    ...
    let emit_toplevel_call name =
       ...
       (* v0.1.56: route the mangled instance name through c_safe_name once,
          exactly like the definition (emit_fn) does. A multi-instantiated name
          whose use site has no concrete arrow keeps the source name -- there is
...
type fn_decl = Monomorph.fn_decl = {
  name      : string;
  param     : string;
  body      : Ast.expr;
  param_ty  : Ast.ty;
...
(* Monomorphization moved to lib/monomorph.ml in v0.1.407, unchanged, so the
   RISC-V backend can be handed monomorphic functions instead of compiling a
   polymorphic `==` as a word comparison. The names below are this backend's
   spelling of that module's; the pass itself has one definition.

   The instantiation table is RETURNED by the pass now rather than written into
   the global below, so a second backend running in the same process (the LSP
   runs several) cannot land in this one's. *)
type fn_skel = Monomorph.fn_skel = {
  sname : string;
  sparam : string;
  sbody : Ast.expr;
...
(* The pass raises without a backend's name on it -- it does not have one. The
   C wording ("unsupported in C codegen subset: ...") is added here, so the
   message a user sees is byte for byte the one this backend has always given.
   EVERY entry point goes through this, not just the one that looked like it
   raised: `lift_fn_skels` also refuses (a multi-arg `let rec` binding), and
...
let lift_fn_skels e = of_monomorph (fun () -> Monomorph.lift_fn_skels e)
let find_live_arrow n t e =
  of_monomorph (fun () -> Monomorph.find_live_arrow n t e)
let specialize_single_use_local_fns e =
  of_monomorph (fun () -> Monomorph.specialize_single_use_local_fns e)
let duplicate_multi_use_local_fns e =
  of_monomorph (fun () -> Monomorph.duplicate_multi_use_local_fns e)
...
let emit_closure_typedef (p : Ast.ty) (r : Ast.ty) : string =
  ...
  (* v0.1.290: a closure carries a COPIER for its env, so a closure can be
     copied out of a dying region the way every other value is. The env used to
     be allocated in the default (program-lifetime) region for exactly one
     reason -- nothing could copy it, since `void* env` is type-erased -- and
     that made every closure a permanent allocation. The copier is generated per
     env type, next to the env's own typedef, which is where the field types are
     known. A closure whose env is NULL, or one built by an FFI adapter over a
    ...
    let fields =
      String.concat "\n"
        (List.map (fun (n, t) ->
          ...
          ce.ce_env_fields)
    in
    (* v0.1.291: the env records its own region, so a copy into the region it is
       already in is a no-op. Without that, copying is neither idempotent nor
...
let native_http_runtime ~tls =
  String.concat "\n"
    ([ "/* --- native HTTP server (Stage 3): single-threaded accept loop, ";
      ...
      "";
      "/* THE LENGTH IS KNOWN AND WAS BEING THROWN AWAY (v0.1.345). This used";
      " * __lang_str_of_cstr, which is strlen -- so a request body stopped at its";
      " * first zero byte. Text bodies have none, so JSON worked and every test";
      " * passed; a FILE UPLOAD does, and a 52-byte PNG arrived as the 8 bytes";
      " * before its first NUL, silently. A Mere str carries its length in a";
      " * header, so it can hold the zero; the length just has to be used. */";
      "static const char* http_current_body(void) { return __lang_str_dup_n(__http_req_body, (size_t)__http_req_body_len); }";
      "static int http_set_status(int c) { __http_status = c; return 0; }";
      "/* v0.1.180: the two the Node host had and this one did not, so a";
      "   server using contrib/http's access_log or arena middleware could";
      "   be built for one host and not the other. */";
      "static int http_current_status(void) { return __http_status; }";
      "/* Per-request arena checkpoint. `with_arena` calls this on entry; the";
      "   accept loop rolls the region back to it once the response has been";
      "   written, so the request's whole working set is reclaimed. Blocks";
      "   allocated past the mark are freed; the bump pointer in the block";
      "   that held the mark goes back to where it was. Opt-in from the Mere";
      ...
      "  __http_req_base = __lang_default_region.base;";
      "  __http_req_cap = __lang_default_region.cap;";
      "}";
      "static int http_arena_mark(void) {";
      "  __http_mark_blocks = __http_req_blocks;";
      "  __http_mark_top = __http_req_top;";
      "  __http_mark_base = __http_req_base;";
      "  __http_mark_cap = __http_req_cap;";
      "  __http_marked = 1; return 0;";
      "}";
      "/* Seconds since the epoch. contrib/http's access_log wants it, and it";
      "   is the last thing standing between a middleware stack and a native";
      "   build. */";
      "static long long unix_time(void) { return (long long)time(NULL); }";
      "";
      "/* Send a file as the whole response, without it passing through a Mere";
      "   str — which matters because a str is NUL-terminated and a file is";
      "   not. Returns 1 when the file was sent and 0 when it could not be";
      ...
      "}";
      "";
      "static void __http_arena_release(void) {";
      "  if (!__http_marked) return;";
      "  __lang_region_block* b = __lang_default_region.blocks;";
      "  while (b && b != __http_mark_blocks) {";
      "    __lang_region_block* prev = b->prev; free(b); b = prev;";
      "  }";
      "  __lang_default_region.blocks = __http_mark_blocks;";
      "  __lang_default_region.base = __http_mark_base;";
      "  __lang_default_region.top = __http_mark_top;";
      "  __lang_default_region.cap = __http_mark_cap;";
      "  __http_marked = 0;";
      "}";
...
let native_ffi_runtime ~tls ~midi ~window ~audio ~filestat ~fdio =
  String.concat "\n"
    [ "/* --- native FFI runtime: Wasm-style byte arena + POSIX TCP --- */";
      ...
      "/* A MERE STR CARRIES ITS LENGTH; strlen throws it away (v0.1.345). These";
      " * three take arbitrary bytes -- a message, a password -- and used strlen,";
      " * so they silently hashed only up to the first zero. Found while checking";
      " * a file upload: the upload was already correct and the CHECKSUM was";
      " * wrong, which is worse, because the instrument agreed with the bug.";
      " *";
      " * The hex helpers next to these keep strlen ON PURPOSE: their input is hex";
      " * text, which has no zero byte, and __lang_str_size on a plain C string";
      " * would read a header that is not there. */";
      "static char* sha256_hex(const char* msg){ unsigned char d[32]; __sha256((const unsigned char*)msg, __lang_str_size(msg), d); return __to_hex(d,32); }";
      ...
      "static char* hmac_sha256_hex_str(const char* key_hex, const char* msg){ unsigned char* k=(unsigned char*)malloc(strlen(key_hex)/2+1); int kn=__from_hex(key_hex,k); unsigned char d[32]; __hmac_sha256(k,kn,(const unsigned char*)msg,(int)__lang_str_size(msg),d); free(k); return __to_hex(d,32); }";
      ...
      "  int pwlen=(int)__lang_str_size(pw); unsigned char* dk=(unsigned char*)malloc(keylen>0?keylen:1);";
  ...
  String.concat "\n"
    [ (* Byte-safe strings (v0.1.x): a Mere `str` is a `char*` pointing at
      ...
      (* MERE_FAIL_TRAP=1 stops here instead of leaving, so a debugger holds the
         ...
         AND THE DEBUGGER IS THE WHOLE ANSWER HERE, because the obvious cheaper
         one does not work. v0.1.486 printed the frames itself, from
         backtrace() + dladdr, and it was green on macOS and empty on Linux:
         dladdr consults the DYNAMIC symbol table there, and the frames a Mere
         program actually stands on are its `__direct` twins, which are emitted
         `static` and are therefore absent from .dynsym with or without
         -rdynamic. Measured in the CI image: `mu_a` and `mu_b` resolve under
         -rdynamic, `mu_a__direct` never does. A diagnostic that answers on one
         platform and says nothing on the other is worse than one that says
         nothing on both, so it says nothing on both -- and a debugger, whose
         unwinder reads DWARF and can see a static function, is handed the job
         through the one line below. *)
...
let region_runtime_helpers =
  String.concat "\n"
    [ "/* A region is a chain of bump-allocated blocks. When the current";
      ...
      "}";
      "";
      "/* Block regions are heap structs, cached on a small per-thread STACK so a";
      ...
      "";
      "   v0.1.295: the cache was ONE deep, and one is not enough as soon as";
      "   regions nest: a loop iteration's region inside a statement's region,";
      "   with statement regions inside the body, releases out of cache order.";
      "   The innermost release filled the single slot, so the next-outer";
      "   release FREED its region -- struct, 1 MB seed and all -- and the next";
      "   acquire at that depth malloc'd them back. One interpreter workload";
      "   paid a 1 MB malloc+free round trip per loop iteration: ~200 GB of";
...
(* Q-120. ONE PLACE THAT SAYS WHAT AN `extern fn` LOOKS LIKE IN C. The program
   emission needs it, and so does `mere --header`, which exists so that a shim
   author does not COPY the layout by hand -- the `mu_` prefix and the field
   order are this generator's choices, and a hand-copied struct is a promise
   nobody checks. Writing the mapping twice would be two answers to one
   question the first time either moved. *)
let c_extern_prototype (name : string) (ty : Ast.ty) : string =
  let rec flatten t =
    match Ast.walk t with
    | Ast.TyArrow (p, r) ->
  ...
  (* v0.1.41: at the FFI boundary `int` stays C `int` even though Mere int is
     `long long` internally. The externs users declare are overwhelmingly
     libc/POSIX functions whose ABI type IS the 32-bit int; declaring them
     `long long` would read undefined upper register bits on arm64. *)
  ...
  in
  Printf.sprintf "extern %s %s(%s);" c_ret name c_args
...
(* Q-120 (b). THE C HEADER A SHIM SHOULD INCLUDE INSTEAD OF COPYING.
   ...
   backends. What it does NOT carry is a way for the author of the other side to
   know what those look like: the `mu_` field prefix, the field order, and the
   fact that a boundary `int` is C's 32-bit `int` and not `long long` are all
   this generator's choices. Shims copied them by hand, which is an ABI held
   together by two people agreeing -- and an `extern` declaration is a promise,
   not a check.

   ⚠ ONLY WHAT THE BOUNDARY REACHES. Every record in the program would leak the
   inside of it into a file the other side compiles against, and would turn any
   internal rename into a break for a shim that never used it. The walk starts
   at the extern signatures.

   ⚠ WHAT IS NOT HERE IS NOT AN OVERSIGHT. Callbacks (a Mere closure as a C
   function pointer), tuples and variants at the boundary are NOT MEASURED, so
   nothing is written about them: a header that named them would be making the
   same unchecked promise this replaces. *)
let emit_c_header (prog : Ast.program) : string =
  let externs =
    List.filter_map (function
      | Ast.Top_extern (name, ty) when not (!is_native_ffi_fwd name) ->
        Some (name, Ast.walk ty)
      | _ -> None) prog.Ast.decls
  in
  (* The record names an extern signature can reach, in first-seen order so the
     output is a function of the program and not of a hash table's iteration. *)
  let needed_records = ref [] in
  let uses_bytes = ref false in
  let rec scan t =
    match Ast.walk t with
    | Ast.TyArrow (a, b) -> scan a; scan b
    | Ast.TyBytes -> uses_bytes := true
    | Ast.TyCon (n, args) ->
      if Hashtbl.mem Typer.records n && not (List.mem n !needed_records) then begin
        needed_records := !needed_records @ [ n ];
        (match Hashtbl.find_opt Typer.records n with
         | Some info -> List.iter (fun (_, ft) -> scan ft) info.Typer.r_fields
         | None -> ())
      end;
      List.iter scan args
    | Ast.TyTuple ts -> List.iter scan ts
    | Ast.TyRef (_, _, inner) -> scan inner
    | _ -> ()
  in
  List.iter (fun (_, ty) -> scan ty) externs;
  let lines =
    [ "/* Generated by `mere --ffi-header`. Do not edit.";
      " *";
      " * Include this from the C side of an `extern fn` instead of writing the";
      " * struct out again: the field prefix, the field order and the width of a";
      " * boundary `int` are the compiler's choices, and a copy of them is a";
      " * promise nobody checks.";
      " *";
      " * A boundary `int` is C's `int` (32-bit), not `long long`.";
      " * This describes the C and LLVM backends. Wasm passes everything as i32";
      " * pointers, which is a different ABI for the same declaration.";
      " */";
      (* ⚠ Not MERE_FFI_H: `mere --header` (Mere as a library) already uses
         that, and a program that includes both would silently get only the
         first. Two headers, two guards. *)
      "#ifndef MERE_EXTERN_H";
      "#define MERE_EXTERN_H";
      "" ]
    @ (if !uses_bytes then
         [ "struct mere_bytes { long long len; unsigned char data[]; };";
    ...
    @ List.concat_map (fun n ->
        let body = emit_record_struct_body n in
        if body = "" then []
        else
          [ Printf.sprintf "typedef struct %s %s;" (c_type_name n) (c_type_name n);
            body; "" ])
        !needed_records
    @ (if externs = [] then []
       else List.map (fun (n, ty) -> c_extern_prototype n ty) externs @ [ "" ])
    @ [ "#endif /* MERE_EXTERN_H */" ]
  in
  String.concat "\n" lines ^ "\n"
...
let emit_program ?(main_ty = Ast.TyInt) (prog : Ast.program) : string =
  ...
  let parts =
    [ (* _GNU_SOURCE BEFORE EVERY HEADER, because that is the only place it works.
      ...
      "     small stack now; drain the lot.) */";
      "  while (__lang_region_cache_n > 0) {";
      "    __lang_region* __rc = __lang_region_cache[--__lang_region_cache_n];";
    ...
    @ (let extern_decls =
         Hashtbl.fold (fun name ty acc ->
           ...
           c_extern_prototype name ty :: acc)
           extern_fn_decls []
       in
