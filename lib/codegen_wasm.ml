...
let rec occurring_names (e : Ast.expr) (acc : string list) : string list =
  ...
  | Ast.Let_rec (bs, b) ->
    List.fold_left (fun ac (_, _, e) -> occurring_names e ac) (occurring_names b acc) bs
...
(* Q-103: the same records the shared monomorphization pass returns, so this
   backend and that pass cannot disagree about a field. *)
...
let rec ty_tag (t : Ast.ty) : string =
  ...
  (* v0.1.409 (Q-103): `bytes` was simply MISSING -- it fell to the catch-all
     below, so this tagger refused to name a type the rest of the backend
     supports (`ty_is_concrete` accepts it and the bytes runtime exists). Same
  ...
  | Ast.TyRef (_, _, inner) ->
    ...
    ty_tag inner
  | other ->
    (* v0.1.409: say WHICH type. This refusal named neither the type nor the
       program, so the only way to find out what it was refusing was to
       instrument the compiler. *)
    raise (Codegen_error (Loc.dummy,
      Printf.sprintf "unsupported Wasm codegen type for ty_tag: %s"
        (Ast.pp_ty other)))
...
(* Q-103: one implementation, in lib/monomorph.ml. This file's copies of
   lift_fn_skels / find_concrete_arrow / find_all_concrete_arrows_in /
   clone_with_fresh_tyvars were byte-for-byte the same logic as that module's
   apart from the word "Wasm" in one refusal, which is now a parameter. Keeping
   near-identical copies is how the Wasm resolver ended up three generations
   behind the C one without anyone deciding that it should be. *)
let lift_fn_skels e = 
  try Monomorph.lift_fn_skels ~subset:"Wasm subset" e with
...
(* Q-103: the shared pass's table, carrying the namer that produced the
   specializations -- see Monomorph.inst_table. `mangled_inst_name_wasm` below
   is that namer, and both the pass and every call site here go through it. *)
let multi_inst_fns_wasm : Monomorph.inst_table ref =
  ref (Monomorph.empty_inst_table ())

(* Q-103: built on the shared pass's tagger, not this file's `ty_tag`. The pass
   names the specializations it emits, so a call site that spelled the name any
   other way would call a function that does not exist. (This backend's own
   `ty_tag` stays -- it names `show_*` / `to_json_*` runtime helpers, a different
   job, and it raises on types the instance namer must still be able to name.) *)
(* The instance namer has to be TOTAL, which `ty_tag` above is not: it refuses
   a residual type variable, and a use site's arrow can still carry one (`'a`).
   The C backend erases those to `int` on a stated rationale -- a type variable
   that survives to codegen is either dead or unconstrained, so nothing ever
   inspects the value and any representation names it -- and the same rationale
   is what makes a NAME for it fine here. `ty_tag` keeps refusing, because for
   its other job (naming `show_*` / `to_json_*` runtime helpers) a variable
   really is a diagnostic.

   `ty_as_tagged` and not `deep_erase_tyvars`: the difference is the REGION slot
   of a Vec / Map / StrBuf / ByteBuf. Erasing an unresolved region to `int`
   names the same type twice -- `Vec_int_int` at a use site whose region never
   resolved, `Vec___heap_int` at the definition whose did -- and the emitted
   module then calls a function it does not define. (That is the mpng P5 shape
   the C backend already had a name for; measured here as
   `undefined function variable $_dc_mul2k__Vec_int_int__int__Vec_int_int`.)
   Both names come out of THIS function, so mapping both through the same rule
   is what makes them agree.

   This only became reachable when the fixpoint was shared: while this backend
   was blind to a second instantiation, no such arrow was ever handed to a
   namer, and three programs that had always compiled started being refused by
   the naming rather than by the code. *)
let mangled_inst_name_wasm (base : string) (arrow : Ast.ty) : string =
  ...
  base ^ "__"
  ^ String.concat "__"
      (List.map (fun t -> ty_tag (Monomorph.ty_as_tagged t)) tys)
...
(* v0.1.409 (Q-103): this backend's own copy of the monomorphization fixpoint is
   gone; it calls the shared pass in lib/monomorph.ml. The copy was not an older
   style of the same thing, it was three generations behind and answered
   questions wrongly:

     - no PRISTINE skeleton table. A single resolution unifies the original fn's
       type variables in place, so once a fn was resolved at one type there was
       no polymorphic skeleton left to instantiate from.
     - no PROMOTION branch: `if Hashtbl.mem resolved s.sname then ()`. A fn
       discovered at a second type later was simply not looked at again.
     - `extra_exprs` folded only the multi-inst specs, not the bodies of
       singly-resolved poly fns -- so a use of poly fn B inside poly fn A's body
       never became visible after A resolved (the v0.1.28 B-P2b addition).

   Those three are one bug from a user's side, and test/parity/
   poly_promote_second_type.mere is it: a helper resolved at int, then reached at
   str through a polymorphic caller that resolves later, answered `MISSED` here
   while the interpreter, C and LLVM all answered `found`. Nothing in the suite
...
let lift_inner_fns_wasm (toplevel_names : string list) (fns : fn_decl list)
  ...
  let rec scan_for_called host_map called_acc (e : Ast.expr) cur_name =
    ...
    (match e.Ast.node with
     ...
     | Ast.Let_rec (bs, b) -> List.iter (fun (_, _, v) -> recurse v) bs; recurse b
...
and emit_expr (e : Ast.expr) : unit =
  ...
  | Ast.Let_rec (bindings, body) ->
    ...
    if List.for_all (fun (n, _, _) -> Hashtbl.mem inner_lifts_wasm n) bindings then
      (wasm_tail_pos := saved_tail;
       wasm_in_top_level_body := saved_top;
       emit_expr body)
    else
  ...
  | Ast.Region_block (_, body) ->
    ...
    let rec guard (ex : Ast.expr) : unit =
      ...
      (match ex.Ast.node with
       ...
       | Ast.Let_rec (bs, b) -> List.iter (fun (_, _, v) -> guard v) bs; guard b
...
let simd_runtime_wasm () =
  ...
  ;; u8x16 lane operations (2c). Boxed v128 in, boxed v128 out; bytes are
  ;; { i32 len @0, data @4 }.
  (func $mere_u8x16_and (param $a8 i64) (param $b8 i64) (result i64)
    (call $__mere_box_v128 (v128.and (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $a8))) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $b8))))))
  (func $mere_u8x16_or (param $a8 i64) (param $b8 i64) (result i64)
    (call $__mere_box_v128 (v128.or (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $a8))) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $b8))))))
  (func $mere_u8x16_xor (param $a8 i64) (param $b8 i64) (result i64)
    (call $__mere_box_v128 (v128.xor (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $a8))) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $b8))))))
  (func $mere_u8x16_sub_sat (param $a8 i64) (param $b8 i64) (result i64)
    (call $__mere_box_v128 (i8x16.sub_sat_u (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $a8))) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $b8))))))
  (func $mere_u8x16_eq (param $a8 i64) (param $b8 i64) (result i64)
    (call $__mere_box_v128 (i8x16.eq (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $a8))) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $b8))))))
  (func $mere_u8x16_swizzle (param $t8 i64) (param $i8 i64) (result i64)
    (call $__mere_box_v128 (i8x16.swizzle (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $t8))) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $i8))))))
  (func $mere_u8x16_shr (param $v8 i64) (param $k i64) (result i64)
    (if (i32.or (i64.lt_s (local.get $k) (i64.const 0)) (i64.gt_s (local.get $k) (i64.const 7)))
      (then (return (call $__lang_fail (i64.const %d)))))
    (call $__mere_box_v128 (i8x16.shr_u (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $v8))) (i32.wrap_i64 (local.get $k)))))
  (func $mere_u8x16_shift_in (param $p8 i64) (param $c8 i64) (param $k i64) (result i64)
    (local $buf i32)
    ...
    (local.set $buf (global.get $__lang_bump))
    (global.set $__lang_bump (i32.add (local.get $buf) (i32.const 32)))
    (v128.store offset=0 align=16 (local.get $buf) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $p8))))
    (v128.store offset=16 align=16 (local.get $buf) (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $c8))))
    (call $__mere_box_v128 (v128.load align=1 (i32.add (local.get $buf) (i32.sub (i32.const 16) (i32.wrap_i64 (local.get $k)))))))
  (func $mere_u8x16_any_true (param $v8 i64) (result i64)
    (i64.extend_i32_u (v128.any_true (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $v8))))))
  (func $mere_u8x16_reduce_add (param $v8 i64) (result i64)
    (local $x v128)
    (local.set $x (i32x4.extadd_pairwise_i16x8_u (i16x8.extadd_pairwise_i8x16_u (v128.load offset=0 align=16 (i32.wrap_i64 (local.get $v8))))))
    (i64.extend_i32_u
      (i32.add (i32.add (i32x4.extract_lane 0 (local.get $x)) (i32x4.extract_lane 1 (local.get $x)))
               (i32.add (i32x4.extract_lane 2 (local.get $x)) (i32x4.extract_lane 3 (local.get $x))))))
  (func $mere_u8x16_load_unchecked (param $b8 i64) (param $i8 i64) (result i64)
    (call $__mere_box_v128 (v128.load align=1 (i32.add (i32.add (i32.wrap_i64 (local.get $b8)) (i32.const 4)) (i32.wrap_i64 (local.get $i8))))))
  (func $mere_u8x16_load (param $b8 i64) (param $i8 i64) (result i64)
    (if (i32.or (i64.lt_s (local.get $i8) (i64.const 0))
                (i64.gt_s (i64.add (local.get $i8) (i64.const 16)) (i64.extend_i32_s (i32.load (i32.wrap_i64 (local.get $b8))))))
      (then (return (call $__lang_fail (i64.const %d)))))
    (call $mere_u8x16_load_unchecked (local.get $b8) (local.get $i8)))
  (func $mere_u8x16_from_bytes (param $b8 i64) (result i64)
...
let emit_program ?(main_ty = Ast.TyInt) ?(component = false) (prog : Ast.program) : string =
  ...
  inner_lift_closure_pending_wasm := [];
  multi_inst_fns_wasm := Monomorph.empty_inst_table ~mangle:mangled_inst_name_wasm ();
  str_data_decls := [];
