...
# alone -- 8 of the 53 cases are exactly that, and turning `affects` into a
...
if [ "${cases:-0}" -lt 53 ]; then
  echo "live_query: FAIL — only ${cases:-0} cases ran; the file declares 53"
  exit 1
fi
if [ "$rc" != "0" ]; then
