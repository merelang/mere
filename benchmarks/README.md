...
| `wordfreq` | read text, count words in a hash table, rank, exit | The shape most short-lived programs actually have, and the one the region model should be **good** at: allocate freely, never give anything back, exit. `bench_stream.mere` is the same answer read a line at a time inside a region, ranked with `vec_sort` — the footprint is the table plus one line. |
...
| `mat4xvec4_f32` | the same kernel with `f32x4` -- four lanes instead of two | **Does a wider lane pay once a narrower one already has?** A vec4 fits in one f32x4, so a vertex costs eight vector operations against the f64 row's sixteen; that is a ceiling of 2x and the row measures how much of it arrives (1.37x). Its answer is single precision and therefore NOT the f64 row's answer -- a separate directory, because run.py's same-answer rule is per directory and is right in both. |
...
| `mat4xvec4_f32` (explicit `f32x4`, v0.1.445) | -- (the language has no scalar f32) | **100 ms** | 131 ms (scalar `float`) | four lanes against the f64 row's two: 1.37x it, 1.9x the scalar double row, and ahead of C. Eight vector operations per vertex against sixteen caps this at 2x, so most of the width arrives. The narrowing at the boundary is paid sixteen times for the matrix and four times per vertex and is not what the row is measuring |
