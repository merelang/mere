...
設計 doc: `aidocs/projects/lang/12_drop_and_with.md` の案 (i)「region は Trivial 厳格、Drop ありは `with` で管理」を実装。
...
詳細は [memory-model.md](memory-model.md) と `aidocs/projects/lang/14_view_types.md` 参照。
...
- メモリモデル / region / view / effect は未実装 (設計は `aidocs/projects/lang/` で進行)
- REPL は単一行入力のみ
