...
`view V[R] of T { ... }` は **region パラメータ付きのデータ宣言**。Phase 2.3 では:

- view 構築は `region { ... }` block 内でのみ可能 (region 外で `V { ... }` を書くと型エラー)
- 構築時、view の region パラメータ `R` は最内側の active region 名に置換される (フィールド型に `&R T` がある場合、別名の region でも自動的にタグが揃う)
- フィールドアクセス `v.f1` と record update `{ v | f1 = e1 }` は record と同じく使える (ただし `&R T` 型のフィールド access は raw R を返す既知制約あり)
...
let n = Node { value = 1, next = 0 } in ...                        // ERROR: 

view Slot[R] { item: &R int };
region S { Slot { item = &S 42 } }    // OK (R → S、フィールドも &S int)
region S { Slot { item = &T 42 } }    // ERROR (region 不一致)
```

**将来 Phase で厳格化される予定**:
- view 値自体に region tag (`&R V` 経由の参照強制)
- フィールド access の region 伝播 (`v.field` で active region を反映した型を返す)
