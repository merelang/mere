...
- **The check is still Phase 1 in scope** (bool + variants + small tuple products). **For int / str / float / tuple / record**, a wildcard arm is required, and its absence is a **warning** rather than an error — that arm of the checker cannot name a missing case, so promoting it would demand `| _ ->` on every match over a scalar rather than point at anything.
