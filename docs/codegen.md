...
| 4.14 | 複雑な pattern (P_int / P_str / P_bool / P_record / nested / P_as) | Match の pattern compilation を `compile_pattern` で fully recursive に書き直し。各 pattern を (test, bindings) に分解、constructor 内に constructor/tuple/record を nest 可能、record pattern も destructure 可能。or-pattern と guard はまだ未サポート |
...
| 機能 | 必要なもの |
|---|---|
| or-pattern + match guard | 両方の選択肢が同じ名前を bind する制約 + guard 条件チェック |
