...
- view / `with` 等のメモリモデル一部 (region と `&R T` は実体動くが、view 構築・close field 呼出は no-op)
...
| view runtime | region 内の view 構築を bump allocator に乗せる、`close` field 自動呼出の codegen 化 |
| GC or proper memory mgmt | 文字列 / closure env / variant node の malloc leak 解消、long-running program 対応 |
