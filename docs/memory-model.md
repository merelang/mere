...
### 動くこと (Phase 2: 構文 + 値式 + escape check + view 宣言 + view 構築の region 強制)
...
- **view の region 強制** (Phase 2.3) — view 構築は region block 内のみ。view の region パラメータ `R` は構築時に最内側の active region 名に置換される。フィールド型に `&R T` を持つ view を別 region で構築すると、フィールドの region tag が自動的に伝播
...
- **view フィールド access の region 伝播** — Phase 2.3 では構築時には R を置換するが、`n.field` で取り出した時の型は宣言時の生の R を返す (使い物にしようとすると型エラーになるため安全ではあるが、UX 上は将来改善)
...
### 現状 (Phase 2.3、2026-06-16)

view 構築は **region block 内に限定** され、宣言時の region パラメータ `R` は構築時に最内側の active region 名に置換される。これによりフィールドに `&R T` がある場合、別名 region でも自動的にタグが揃う。
...
view Slot[R] { item: &R int };
region S { let s = Slot { item = &S 42 } in 100 }               // 100 (R → S)
region S { let s = Slot { item = &T 42 } in 100 }               // ERROR (region 不一致)
...
- view 値そのものを `&R V` 経由でしか参照できない (型の region tag が view 自体にも乗る)
- フィールド access の region 伝播 (Phase 2.3 では生の R が返るが、将来 active region に置換)
...
- [ ] `Trivial[R]` 型制約
- [ ] view 値の type-level region tag (`&R V` 経由の参照強制 + フィールド access の region 伝播)
