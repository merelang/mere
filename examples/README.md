...
| **Phase 54 (2026-07-01) additions — web backend Stage A** | |
| [http_echo_server.mere](http_echo_server.mere) ⭐ | minimal HTTP server: registers a Mere closure via `extern fn http_serve: int -> (str -> str) -> unit;`, Node host glue in [contrib/http/](../contrib/http/) dispatches each request back into Mere and returns the string as the response body. Wasm only; run with `node scripts/run_http_server.js` |
| [http_echo_body.mere](http_echo_body.mere) | POST/PUT-capable echo: pulls the request body via `http_current_body ()` and mirrors both the request line and body. Verifies the per-request body slot pattern end to end |
| [http_json_api.mere](http_json_api.mere) ⭐ | small JSON REST API: routes on the request line, returns `application/json` via `http_set_content_type`, and `404` for unknown paths via `http_set_status`. Six endpoints incl. `GET /users/1` and `POST /echo` |
| [http_todo_api.mere](http_todo_api.mere) ⭐ | in-memory TODO CRUD REST API. POST creates, GET lists / fetches, PUT updates, DELETE removes (with 404 on missing id); state lives in a top-level mutable `Map[str, str]` (Phase 30.2 pattern) with a counter for id assignment. Round-trip from `curl -X POST -d ...` through `http_current_body` + `strbuf` copy into the bump arena |
| [http_docs_server.mere](http_docs_server.mere) | serves the mere repo's own docs (README, changelog, tutorial, reference, LICENSE) over HTTP by combining `contrib/http` with the builtin `read_file`. Real static file serving; bytewise-identical output verified with `wc -c` |
| [http_fmt_service.mere](http_fmt_service.mere) ⭐ | **full-stack dogfood**: the self-host formatter (`contrib/parser` + `contrib/fmt`) exposed as an HTTP service via `contrib/http`. `POST /fmt` with Mere source in the body returns the pretty-printed source. Mere formatting Mere, over HTTP, from inside a Mere-compiled wasm binary. Compiles to 951KB WAT / 110KB wasm |
| [http_fmt_webapp.mere](http_fmt_webapp.mere) ⭐ | **single-file web app**: same fmt pipeline as `http_fmt_service`, plus `GET /` serves [http_fmt_webapp.html](http_fmt_webapp.html) via `read_file` for a live in-browser formatter. Point a browser at `localhost:8080` and get a working formatter UI — both the HTML and the API from one wasm binary |
