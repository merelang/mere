...
- **2693 tests passing** (`dune test`), plus cross-backend parity (163 programs) and ~30 differential gates against external oracles in CI. These counts are checked against the suite by `scripts/first_run_check.sh`, because a number in a README rots silently
