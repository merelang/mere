...
- **2588 tests passing** (`dune test`), plus cross-backend parity (133 programs) and ~30 differential gates against external oracles in CI. These counts are checked against the suite by `scripts/first_run_check.sh`, because a number in a README rots silently
...
# and the interpreter, C and LLVM agree byte for byte. Wasm miscompiles it
# today (Q-068) so that backend is not claimed here.
