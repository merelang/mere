...
旧仮称: `lang-ml` (2026-06-19 に Mere に確定、[NAMING.md](https://github.com/284km/aidocs/blob/main/projects/lang/NAMING.md) 参照)。
...
| codegen | C / LLVM IR / Wasm (WAT) の 3 backend が parity で動く (詳細は [codegen.md](docs/codegen.md))。`Vec[R, T]` は Phase 15.2 / 15.3 / 15.4 で 3 backend すべてで要素型一般化済み (int / bool / str / tuple / record / variant)、Phase 15.5 / 15.6 で高階 API 主要 5 つ (vec_set / iter / fold / map / filter) も 3 backend 対応、Phase 15.7 で OwnedVec[T] + vec_to_owned / owned_vec_to_vec も 3 backend 対応、Phase 15.9 で StrBuf[R] も、Phase 15.10 で **Map[R, K, V] (K = int / str + V 任意)** も 3 backend 対応 |
