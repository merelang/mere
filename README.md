...
- **2748 tests passing** (`dune test`), plus cross-backend parity (178 programs) and ~30 differential gates against external oracles in CI. The parity count is re-derived from `test/parity/` by `scripts/first_run_check.sh`, because a number in a README rots silently — and the test count is not, which is how it sat at 2715 while the suite passed 2724 (v0.1.470). It is stated here as of v0.1.482 and nothing checks it
