...
| codegen | C / LLVM IR / Wasm (WAT) の 3 backend が parity で動く (詳細は [codegen.md](docs/codegen.md))。Vec/OwnedVec/StrBuf/Map は interpreter-only (例外: C codegen のみ `Vec[R, int]` を Phase 15.1 で対応開始) |
...
(Vec / OwnedVec / StrBuf / Map は interpreter-only、codegen 対応は今後。
Phase 15.1 で C codegen のみ `Vec[R, int]` を先行対応 — 例:
`examples/vec_codegen_c.mere`)
